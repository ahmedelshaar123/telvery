...
@section('content')
    ...
    <!-- form start -->
        {!! Form::model($user,[
                                'action'=>['App\Http\Controllers\Admin\UserController@update', $user->id],
                                'id'=>'myForm',
                                'role'=>'form',
                                'method'=>'PUT',
                                'files'=>'true'
        ...
        <div class="box-body">
                ...
                @if(auth()->user()->hasRole('admin'))
                        ...
                        <div class="form-group">
                            ...
                            <select name="category_id" class="form-control" required>
                                ...
                                @foreach($categories as $category)
                                    <option {{$category->id == optional($user->category)->id ? 'selected' : ''}} value="{{$category->id}}">{{$category->name_ar}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            {!! Form::date('company_age',$user->company_age,[
                                'class'=>'form-control',
                            ]) !!}
                        </div>
                    </div>
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        {!! Form::textarea("desc_ar",$user->desc_ar,[
                            'class'=>'form-control',
                        ]) !!}
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        {!! Form::textarea("desc_en",$user->desc_en,[
                            'class'=>'form-control',
                        ]) !!}
                    </div>
                </div>
            ...
            <div class="form-group">
                ...
                {!! Form::url("facebook_url",$user->facebook_url,[
                    'class'=>'form-control',
                ]) !!}
            </div>
            <div class="form-group">
                <label for="twitter_url">رابط التويتر</label>
                {!! Form::url("twitter_url",$user->twitter_url,[
                    'class'=>'form-control',
                ]) !!}
            </div>
            <div class="form-group">
                <label for="instagram_url">رابط الانستجرام</label>
                {!! Form::url("instagram_url",$user->instagram_url,[
                    'class'=>'form-control',
                ]) !!}
            </div>
            <div class="form-group">
                <label for="google_url">رابط الجوجل</label>
                {!! Form::url("google_url",$user->google_url,[
                    'class'=>'form-control',
                ]) !!}
            </div>
            <div class="form-group">
                            ...
                            @foreach($models[$x] as $model)
                                <div class="tab-pane {{ $x == 0 ? 'active' : '' }}" id="{{$model}}">
                                    @foreach($maps[$x] as $map)
                                        <label><input type="checkbox" name="permissions[]" value="{{$map.'_'.$model}}" {{ $user->hasPermission($map.'_'.$model) ? 'checked' : '' }}> @lang('messages.'.$map)</label>&nbsp;
                                    @endforeach
                                </div>
                            @endforeach
                        @endfor
